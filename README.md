# XM Cloud Front End Application Starter Kitss
...
This repository provides starter kits for the legacy JSS applications and for the newer Content SDK (https://doc.sitecore.com/xmc/en/developers/content-sdk/sitecore-content-sdk-for-xm-cloud.html).
...
- Create a new `.env.local` file in the `./headapps/content-sdk/nextjs-starter` (use `./headapps/jss/nextjs-starter` for JSS) folder of this repository and paste the contents from your clipboard.
- Run the following commands in the root of the repository to start the NextJs application:
  ...
  npm run dev
  ```
  (use `npm run start:connected` as the last command for JSS)
